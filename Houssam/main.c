...
void afficher_texte(SDL_Renderer *renderer, char* pseudoDemande, int posX, int posY, int taille);
//char * recuperer_pseudo_clavier(SDL_Renderer *renderer);
void creation_joueurs(SDL_Renderer *renderer, JOUEUR *joueurs);
...
int main(int argc, char *argv[])
{
    ...
    creation_joueurs(renderer, joueurs);
...
void afficher_texte(SDL_Renderer *renderer, char* pseudoDemande, int posX, int posY, int taille)
{
    ...
    TTF_Font* verdana = TTF_OpenFont("src/verdana.ttf", taille);
    if (verdana == NULL){
	...
	SDL_CreationRectangleRempli(renderer,0, 200, 800, 100); 
	SDL_Surface* surfaceMessage = NULL ;
	
	
    surfaceMessage = TTF_RenderText_Solid(verdana, pseudoDemande, gold);    
...
void creation_joueurs(SDL_Renderer *renderer, JOUEUR *joueurs) {
	...
	afficher_texte(renderer, "Veuillez entrez votre pseudo (20 caractères max)", 5, 5, 10);
	...
	for (i = 0; i < nbJoueurs; i++) {
		...
		while( program_launched )
		{
			...
			{			
				...
				if( (event.key.keysym.sym == SDLK_BACKSPACE) && (index>0) )
				{ 
					printf("HHHHHH 2\n");
					pseudo[index-1]=' ';
					index-=1;
				...
				if( (event.key.keysym.sym!=SDLK_BACKSPACE) && (event.key.keysym.sym!=SDLK_RETURN) && (index<(20)) )
				{
					printf("HHHHHH 3\n");
					pseudo[index]=event.key.keysym.sym;
					index+=1;
				}
				...
				if( (event.key.keysym.sym==SDLK_RETURN) && (nbJoueurs != 0) )
				{
					printf("HHHHHH 4\n");
					program_launched = SDL_FALSE;
				}
				
				afficher_texte(renderer, pseudo, 10, 200, 30);
				SDL_RenderPresent(renderer);
			}
